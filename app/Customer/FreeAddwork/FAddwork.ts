...
export const addWork = async (formData: FormData) => {
    ...
    if (mainImgFile) {
      const mainImgFileName = `${uuidv4()}${mainImgFile.name.substring(mainImgFile.name.lastIndexOf('.'))}`;
      ...
      .insert([
        {
          ...
          work_detail,
          work_ex: fileUrls,
          work_mainimg: mainImgUrl,  // Store the main image separately
          work_Exdetail,
          work_budget,
          work_catagory,
          work_deadline,

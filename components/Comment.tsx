...
export default function Comment() {
  ...
  return (
    <div className="mt-5 text-secondary flex flex-col gap-5 w-[50%] max-lg:w-full">
        ...
        {data.map((index, note) => (
            <div className="border border-secondary bg-white p-3 rounded-lg shadow-xl" key={note}>
            <div className="flex gap-2 items-center">
                ...
                height={40}
                />
                <h1 className='text-primary font-medium'>{index.name}</h1>
            </div>
            <hr className="border-[#D9D9D9] my-2" />
            <p className='m-2'>{index.title}</p>
            </div>
        ))}
    </div>
  );
